...
export default class ItemView extends View {
        ...
        <div class="${pfx}layer-item-left">
          <div class="${clsTitleC}">
            <div class="${clsTitle}" style="padding-left: ${gut}">
              <div class="${pfx}layer-title-inn ${level === 0 ? `${pfx}layer-body`: level === 1 ? `${pfx}layer-head`:`${pfx}layer-child`}" title="${name}">
                <i class="${this.clsCaret}" data-toggle-open>${chevron}</i>
                  ${icon ? `<span class="${clsBase}__icon">${icon}</span>` : ''}
                  <div style="display: flex; flex-direction: row; align-Items: center">
                  <i style="margin-right: 8px;" class="${iconObject[this.model.attributes.tagName as keyof typeof iconObject]}"></i>       
  ...
  constructor(opt: ItemViewProps) {
    ...
    this.className = `${pfx}layer ${pfx}layer__t-${type} no-select ${ppfx}two-color ${opt.level === 1 ? `${pfx}layer-border`:``}`;
