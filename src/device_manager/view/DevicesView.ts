...
export default class DevicesView extends View {
  ...
  changeDevice(id: string) {
    this.em.Devices.select(id);
  }

  template({ ppfx, label, em }: { ppfx: string; label: string, em: EditorModel}) {
    
    return html `
    <div class="${ppfx}device-newbox">
    <div class="selectDesktop ${em.getDeviceModel()?.attributes.id === "desktop" ? `${ppfx}device-newbox-item-active` : `${ppfx}device-newbox-item-inactive`}">
    <i class="fa-solid fa-desktop"></i>
    </div>
    <div class="selectMobile ${em.getDeviceModel()?.attributes.id === "mobilePortrait" ? `${ppfx}device-newbox-item-active` : `${ppfx}device-newbox-item-inactive`}">
  ...
  events() {
    return {
      'click .selectDesktop': 'selectDesktop',
      'click .selectMobile': 'selectMobile',
      change: 'updateDevice',
  ...
  constructor(o: { config: DevicesViewConfig; collection: Devices }) {
    ...
    this.em.getDeviceModel()?.on('change:id', this.render, this);
  }
  ...
  selectDesktop() {
    this.em.Devices.select('desktop');
    this.render();  }
  
  selectMobile() {
    this.em.Devices.select('mobilePortrait');
    this.render();  }
  ...
  render() {
    ...
    $el.html(this.template({ ppfx, label, em }));
